...
var runner = require('minigun-core').runner;
var fs = require('fs');
  ...
  async.waterfall([
      function readScript(callback) {
        ...
        fs.readFile(scriptPath, 'utf-8', function(err, data) {
          ...
          var script;
          try {
            var scriptTemplate = JSON.parse(data);
            if (config.has('minigun')) {
              script = object(scriptTemplate, config.get('minigun'));
            } else {
    ...
    function(err, result) {
      ...
      ee.on('phaseStarted', function(opts) {
        console.log(
          ...
          opts.index, (opts.name ? ' (' + opts.name + ')' : ''),
          opts.duration);
